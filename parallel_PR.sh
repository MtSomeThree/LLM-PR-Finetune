...
do
	...
	let end=(i+2)*10000
	{
		python train_nado.py --cuda $cuda --num_epochs 5 --save_dir "$prefix"/NADO_"$i"_5.pt --load_dir "$prefix"/PR_"$j"_3.pt --load_dir2 "$prefix"/NADO_"$j"_5.pt --samples_file dump/"$prefix"/PR_"$k"_3_"$last"-"$start"-200-20-API.pt --temperature $temp
