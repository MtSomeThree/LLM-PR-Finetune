...
class NADO_Llama(LlamaForCausalLM):
    _keys_to_ignore_on_load_missing = [r"NADO"]
    def __init__(self, config):
        super().__init__(config)
        self.NADO_config = copy.deepcopy(config)
        self.NADO_config.num_hidden_layers = 8
        self.NADO = LlamaModel(self.NADO_config)
        self.NADO_linear = nn.Linear(self.NADO_config.hidden_size, self.NADO_config.vocab_size, bias=True)
...
if __name__ == '__main__':
    ...
    model = NADO_Llama.from_pretrained("../llama/7B").to(device)
